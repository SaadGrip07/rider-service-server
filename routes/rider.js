...
router.post("/rider-register", upload, async (req, res) => {
  ...
  try {
    ...
    if (altPhoneCheckResult.recordset.length > 0) {
      return res
        .status(400)
        .json({ message: "Alternative mobile number is already registered." });
    }
    ...
    if (CNICCheckResult.recordset.length > 0) {
      return res
        .status(400)
        .json({ message: "This CNIC number is already registered." });
    }
    // Upload images to Cloudinary
    const profileImageUpload = await uploadToCloudinary(
      profileImage[0].buffer,
      "Riders/ProfileImages",
      `${cnicNumber}-profileImage`
    );
    const cnicFrontImageUpload = await uploadToCloudinary(
      cnicFrontImage[0].buffer,
      "Riders/CNICImages",
      `${cnicNumber}-cnicFrontImage`
    );
    const cnicBackImageUpload = await uploadToCloudinary(
      cnicBackImage[0].buffer,
      "Riders/CNICImages",
      `${cnicNumber}-cnicBackImage`
    );
    const licenseFrontImageUpload = await uploadToCloudinary(
      licenseFrontImage[0].buffer,
      "Riders/LicenseImages",
      `${cnicNumber}-licenseFrontImage`
    );
    const licenseBackImageUpload = await uploadToCloudinary(
      licenseBackImage[0].buffer,
      "Riders/LicenseImages",
      `${cnicNumber}-licenseBackImage`
    );
    const bikeFrontImageUpload = await uploadToCloudinary(
      bikeFrontImage[0].buffer,
      "Riders/BikeImages",
      `${cnicNumber}-bikeFrontImage`
    );
    const bikeBackImageUpload = await uploadToCloudinary(
      bikeBackImage[0].buffer,
      "Riders/BikeImages",
      `${cnicNumber}-bikeBackImage`
    );
    const bikeLeftImageUpload = await uploadToCloudinary(
      bikeLeftImage[0].buffer,
      "Riders/BikeImages",
      `${cnicNumber}-bikeLeftImage`
    );
    const bikeRightImageUpload = await uploadToCloudinary(
      bikeRightImage[0].buffer,
      "Riders/BikeImages",
      `${cnicNumber}-bikeRightImage`
    );
    ...
    request.input(
      "CnicFrontImage",
      sql.NVarChar,
      cnicFrontImageUpload.secure_url
    ); // Cloudinary URL
    request.input(
      "CnicBackImage",
      sql.NVarChar,
      cnicBackImageUpload.secure_url
    ); // Cloudinary URL
    ...
    request.input(
      "LicenseFrontImage",
      sql.NVarChar,
      licenseFrontImageUpload.secure_url
    ); // Cloudinary URL
    request.input(
      "LicenseBackImage",
      sql.NVarChar,
      licenseBackImageUpload.secure_url
    ); // Cloudinary URL
    ...
    request.input(
      "BikeFrontImage",
      sql.NVarChar,
      bikeFrontImageUpload.secure_url
    ); // Cloudinary URL
    request.input(
      "BikeBackImage",
      sql.NVarChar,
      bikeBackImageUpload.secure_url
    ); // Cloudinary URL
    request.input(
      "BikeLeftImage",
      sql.NVarChar,
      bikeLeftImageUpload.secure_url
    ); // Cloudinary URL
    request.input(
      "BikeRightImage",
      sql.NVarChar,
      bikeRightImageUpload.secure_url
    ); // Cloudinary URL
    ...
    res
      ...
      .json({
        success: true,
        message:
          "Dear Rider your request for registration successfully Sent ✅. Please wait for Admin side Confirmation.",
      });
  } catch (error) {
    console.error("Error registering rider:", error);
    res
      .status(500)
      .json({ message: "An error occurred while registering the rider." });
  }
});
//
...
router.post("/rider-exist", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "No Record Found for this Rider" });
  } catch (error) {
    console.error("Error registering rider:", error);
    res
      .status(500)
...
router.post("/rider-login", async (req, res) => {
  ...
  try {
    ...
    const result = await request.query(
      `SELECT * FROM RidersDetail WHERE MN = '${mobileNumber}'`
    );
    ...
    if (rider.EMSR === "New-Registration") {
      return res
        .status(401)
        .json({ message: "Your Registration is in Process" });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Oops Server Side Error!" });
  }
});
  ...
  try {
    ...
    if (checkRiderCNIC.recordset.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "There is no rider with this CNIC number.",
        });
    }
    ...
    if (checkExistingUID.recordset.length > 0) {
      return res
        .status(400)
        .json({ success: false, message: "This UID is already assigned." });
    }
    ...
    res
      .status(201)
      .json({ success: true, message: "Rider hired Successfully." });
  } catch (err) {
...
router.delete("/rider-delete-admin", async (req, res) => {
  ...
  if (!riderCNIC) {
    return res
      .status(400)
      .json({ success: false, message: "riderCNIC is Required!" });
  }
  try {
    await RMDBConnect;
    const request = new sql.Request();
...
router.get("/active-riders", async (req, res) => {
  try {
    ...
    if (result.recordset.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No active riders found." });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching active riders" });
  }
});
...
router.post("/rider-register-admin", upload, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Rider added Successfully" });
  } catch (error) {
    console.error("Error registering rider:", error);
    res
      .status(500)
...
router.put("/update-rider-details", upload, async (req, res) => {
  ...
  try {
    ...
    if (profileImage) {
      ...
      params.push({
        name: "ProfileImage",
        value: profileImageUpload.secure_url,
      });
    }
    if (cnicFrontImage) {
      const cnicFrontImageUpload = await uploadToCloudinary(
      ...
      params.push({
        name: "CnicFrontImage",
        value: cnicFrontImageUpload.secure_url,
      });
    }
    if (cnicBackImage) {
      const cnicBackImageUpload = await uploadToCloudinary(
      ...
      params.push({
        name: "CnicBackImage",
        value: cnicBackImageUpload.secure_url,
      });
    }
    if (bikeFrontImage) {
      const bikeFrontImageUpload = await uploadToCloudinary(
    ...
    res
      .status(200)
      .json({ success: true, message: "Rider details updated successfully." });
  } catch (error) {
    console.error("Error updating rider details:", error);
    res
      .status(500)
      .json({ message: "An error occurred while updating rider details." });
  }
});
